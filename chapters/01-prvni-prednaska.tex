...
V objektovém modelu jsou hlavní prvky, jenž reprezentují data, \textit{objekty}, tedy serializované entity, které lze interpretovat pomocí nějakého vyššího programovacího jazyka. Prvním jazykem podporujícím takovéto pojetí byl Common Lisp.
...
Mějme relační schéma $R\subseteq Y$, pak relace nad $R$ je libovolná konečná podmnožina na kartézském součinu domén atributů z $R$, tedy
$$
...
\begin{description}
\item[Skalární], které jsou opozitem hodnotových typů ze známějších jazyků a patří sem například\upinlinecode{TutorialD}{!}{BOOLEAN},\upinlinecode{TutorialD}{!}{INTEGER} nebo\upinlinecode{TutorialD}{!}{STRING} a také uživatelsky definované typy.
...
\begin{enumerate}
\item[Hodnoty] - jednotlivé konstanty, relace a n-tice. Nemohou se měnit - \uv{Jako ve Scheme}
